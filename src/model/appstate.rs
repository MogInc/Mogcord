...
impl AppState
{
    ...
    ) -> Arc<Self>
    {
        ...
        let refresh_tokens =
            Arc::clone(&db) as Arc<dyn refresh_token::Repository>;
        let relations = Arc::clone(&db) as Arc<dyn relation::Repository>;

        let logs = Arc::new(FileWriter::new(
            log_path.to_string(),
