...
) -> error::Result<'err, ()>
{
    ...
    let claims = auth::extract_acces_token(
        &acces_token_cookie,
        &TokenStatus::AllowExpired,
    )
    .map_err(|err| {
        server_error!(
            err,
    ...
    let refresh_token_cookie = jar
        .get_cookie(auth::CookieNames::AUTH_REFRESH.as_str())
        .map_err(|err| {
            server_error!(
                err,
                error::Kind::NoAuth,
    ...
    if !refresh_token.owner.flag.is_allowed_on_mogcord()
    {
        ...
        .add_debug_info(
            "user flag",
            refresh_token.owner.flag.to_string(),
        ));
    }
