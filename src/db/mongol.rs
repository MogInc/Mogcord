...
impl MongolDB
{
    ...
    {
        let owner_index = IndexModel::builder().keys(doc! { "owner_id": 1}).build();

        let users_index = IndexModel::builder().keys(doc! { "user_ids": 1}).build();

        let channels_index = IndexModel::builder().keys(doc! { "channel_ids": 1}).build();
